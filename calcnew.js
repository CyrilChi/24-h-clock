function calcnew(type, latitude, longitude, year, mos, day) {
  ...
  var JD = calcJD(parseFloat(year), parseFloat(mos+1), parseFloat(day));
  var dow = calcDayOfWeek(JD);
  var doy = calcDayOfYear(parseFloat(mos+1), parseFloat(day), parseFloat(year));
...
City[j++] = new city('柏林,德国', 52.33, -13.3, -1);
City[j++] = new city('孟买,印度', 18.9333, -72.8333, -5.5);
City[j++] = new city('布宜诺斯艾利斯, 阿根廷', -34.6, 58.45, 3);
City[j++] = new city('开罗,埃及', 30.1, -31.3667, -2);
City[j++] = new city('开普敦,南非', -33.9167, -18.3667, -2);
City[j++] = new city('加拉加斯,委内瑞拉', 10.5, 66.9333, 4);
...
City[j++] = new city('伦敦,英国', 51.5, 0.1667, 0);
...
City[j++] = new city('圣保罗,巴西', -22.9, 43.2333, 3);
City[j++] = new city('利雅得,沙特阿拉伯', 24.633, -46.71667, -3);
City[j++] = new city('罗马,意大利', 41.9, -12.4833, -1);
City[j++] = new city('悉尼,澳大利亚', -33.8667, -151.2167, -10);
City[j++] = new city('东京,日本', 35.7, -139.7667, -9);
...
function convLatLong(f) {
  ...
  if (neg != 1) {
    var latSeconds =
      parseFloat(f['latDeg'].value) * 3600 + parseFloat(f['latMin'].value) * 60 + parseFloat(f['latSec'].value) * 1;

    f['latDeg'].value = Math.floor(latSeconds / 3600);
    f['latMin'].value = Math.floor((latSeconds - parseFloat(f['latDeg'].value) * 3600) / 60);
    f['latSec'].value = Math.floor(
      latSeconds - parseFloat(f['latDeg'].value) * 3600 - parseFloat(f['latMin'].value) * 60 + 0.5
    );
  ...
  if (neg != 1) {
    var lonSeconds =
      parseFloat(f['lonDeg'].value) * 3600 + parseFloat(f['lonMin'].value) * 60 + parseFloat(f['lonSec'].value) * 1;
    f['lonDeg'].value = Math.floor(lonSeconds / 3600);
    f['lonMin'].value = Math.floor((lonSeconds - parseFloat(f['lonDeg'].value) * 3600) / 60);
    f['lonSec'].value = Math.floor(
      lonSeconds - parseFloat(f['lonDeg'].value) * 3600 - parseFloat(f['lonMin'].value) * 60 + 0.5
    );
...
function calcHourAngleSunrise(lat, solarDec) {
  ...
  var HA = Math.acos(
    Math.cos(degToRad(90.833)) / (Math.cos(latRad) * Math.cos(sdRad)) - Math.tan(latRad) * Math.tan(sdRad)
  );
